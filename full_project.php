...
<strong id="first" data-post-id='<?php echo title_for_id('مخطط لصفحات موقع قاموس "مصطلحات الكمبيوتر المصور"', 'publish'); ?>' data-loged-uid='<?php echo  get_current_user_id(); ?>'>
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('تمهيد', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('فيديوهات', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('أنشطة', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('مفاهيم اساسية خاصة بقواعد البيانات', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('انشاء قاعدة بيانات باستخدام برنامج MySQL', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('اختبار (2)', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('اختبار (3)', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
<p><span style='font-size:16.0pt;font-family:Symbol;color:red'>·</span>
<a class="page_link" href="http://www.4themers.com/eman2/database-relation/" title="يحتوى الدرس على شرح انواع العلاقات فى قواعد البيانات

<?php
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('أنواع العلاقات في قواعد البيانات', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('اختبار (4)', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
<img border=0 width=32 height=30 id="Picture 263" src="project_files/image004.png">
<a class="page_link" href="http://www.4themers.com/eman2/create-database-video/" title="يحتوى الرابط على فيديوهات لشرح مرحلة إنشاء جداول قاعدة البيانات
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('خطوات تحميل تطبيق Expression Web على جهازك.', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('إنشاء صفحة الاتصال بقاعدة البيانات', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('اختبار (5)', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('إنشاء صفحة راس الصفحة بقاعدة البيانات Header.php', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('إنشاء صفحة إدخال مصطلح بقاعدة البيانات Add_Term.php', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('اختبار (6)', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
// copy this and paste it in the title of each element change the title in the current_post_id
// give the post title to this function to get the post id
$current_post_id = title_for_id('الاختبار النهائي', 'publish');
$viewsCount = $wpdb->get_var("SELECT count(meta_key) FROM wp_postmeta WHERE meta_key = '_post_view' AND post_id=$current_post_id AND meta_value=$loged_user_id");
echo 'عدد الزيارات: ' . $viewsCount . ' |   ';
...
//Get the current time in Unix.
if (!empty($view_dates)) {
$currentTime = $view_dates; $hoursToAdd = 2; $secondsToAdd = $hoursToAdd * (60 * 60); $newTime = $currentTime + $secondsToAdd;
$last_vist_time = date("d/m/y H:i:s", $newTime);
} else {
  $last_vist_time = 'لا يوجد';
}
  ...
  $('.page_link').click(function(){
    ...
    var loged_id = event.target.getAttribute('data-loged-uid');
                        jQuery.ajax({
                            ...
                            //!! add the path  to ajax_handle.php file Important step
...
$sql = "INSERT INTO wp_postmeta (post_id, meta_key, meta_value)
VALUES ($post_id, $loged_id, $the_time)";
if ($conn->query($sql) === TRUE) {
  $message = "New";
} else {
  $message =  "Error";
